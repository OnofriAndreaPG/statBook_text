...
La funzione di densità (PDF) rappresentata nella Figura -@fig-figName50b è la gaussiana ed è così comune in agricoltura e biologia che è solitamente nota come **curva normale**. L'equazione è:
...
Guardando la curva di Gauss possiamo notare che ha una forma 'a campana' che dipende da solo da $\mu$ e $\sigma$ (Fig. -@fig-figName51).

```{r fig-figName51, echo=FALSE, out.width = '90%', fig.cap = "Distribuzioni normali con diversa media e deviazione standard (rispettivamente 5 e 1 a sinistra, 6 e 3 a destra)", fig.align='center', fig.dim = c(6,3)}
g1 <- ggplot() +
...
La densità non è una probabilità reale e non è interessante di per sé (almeno, non in termini assoluti), ma la PDF ha il vantaggio di restituire la probabilità di un intervallo di eventi come Area Sotto la Curva (Area Under the Curve: AUC). Ad esempio, la Figura -@fig-figName52 evidenzia l'AUC tra 11 e 12, che rappresenta la probabilità di campionare una percella con una produzione in quell'intervallo, partendo da una popolazione di parcelle con media pari a 11 e deviazione standard pari a 1.5.

```{r fig-figName52, echo=FALSE, out.width = "75%", fig.cap = "Area-Under-the-Curve per una PDF gaussiana, che descrive la probabilità di campionare una parcella che ha una produzione nell'intervallo tra 11 e 12, quando la media è 11 e la deviazione standard è 1.5", fig.align='center'}
mean <- 11
sigma <- 1.5
...
Di conseguenza, l'AUC e, quindi, la probabilità, possono essere calcolate integrando la curva gaussiana su un intervallo; in relazione alla Figura -@fig-figName52, la probabilità tra 11 e 12 è 0.248 (vedere il riquadro 5.2). Più semplicemente, la funzione `pnorm()` restituisce l'AUC tra $-\infty$ e qualsiasi valore $y$ (**Funzione di distribuzione cumulativa**; vedere la Figura -@fig-figName52c) e può essere utilizzata per evitare il calcolo di un integrale.
...
```{r fig-figName52c, echo=FALSE, out.width = "75%", fig.cap = "Probabilità cumulativa gaussiana, considerando una distribuzione con media pari ad 11 e deviazione standard pari a 1.5", fig.align='center'}
ggplot() +
