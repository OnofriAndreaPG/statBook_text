...
```{r fig-figName242, echo = F, out.width='90%', fig.align ='center', fig.cap= "Esempio di un semplice grafico a barre in R."}
# Il codice non viene visualizzato, solo la figura
# Il riquadro di codice 3.3 viene mostrato, ma non eseguito
library(ggplot2)
ggplot(data = descript) +
  geom_col(mapping = aes(y = Media, x = row.names(descript)), width = 0.75,
         color = "black", fill = "lightgrey") +
...
```{r echo=F, fig-figName241, out.width='90%', fig.align ='center', fig.cap="Un semplice boxplot con R; i tre gruppi rappresentano tre specie di infestanti (CHEAL: Chenopodium album, CHEHY: C. hybridum e CHEPO: C. polyspermum), contate in 20 griglie disposte casualmente in un campo dell'Italia centrale."}
# rm(list = ls())
